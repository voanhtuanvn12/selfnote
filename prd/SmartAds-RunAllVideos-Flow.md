...
4. [Phase 1 — 72h Learning (Max Delivery)](#4-phase-1--72h-learning-max-delivery)
...
| **Budget_initial** | Ngân sách khởi tạo campaign đầu tiên (tính từ MDB) |
...
[Tính Budget_initial]
        │
        ▼
[Mode: MAX DELIVERY]  ──────────────────────────────────────►  [CRITICAL Monitor]
        │                                                              │ (song song, mọi lúc)
        │ chờ 72h
        ▼
[Mốc 72h]
        ...
        │
        ├── Có sản phẩm >= 20 đơn? ──► ĐÚNG ──► [Tách camp Hero] ──► [Mode: TARGET ROI]
        │                                                                      │
        │                                                         [Vòng lặp 24h — Target ROI]
        │
        ...
        │                                                    Check GMV_daily = 0?
        │                                             GMV>0 ──► [Mode: TARGET ROI]
        │                                             GMV=0 (3 ngày liên tiếp) ──► [PAUSE CAMP]
        │
        └── DB ≥ Budget_initial ─┬── ROI thấp ──► [Giảm ngân sách 20%] ──► [Mode: TARGET ROI]
                                 └── ROI tốt  ──► [Tăng ngân sách 20%] ──► [Mode: TARGET ROI]
                                                                                    │
                                                                                    ▼
                                                                   [Vòng lặp 24h — Target ROI]
                                                                       (xem chi tiết Phase 2)
```
...
- Mode: **Max Delivery** (không đặt ROI constraint ở giai đoạn này)
- Đồng thời: khởi động **CRITICAL Monitor** chạy song song

---

## 4. Phase 1 — 72h Learning (Max Delivery)

> **Không có điều chỉnh nào trong 72h này.** Chỉ quan sát. CRITICAL Monitor là ngoại lệ duy nhất được can thiệp.
...
- **Có sản phẩm ≥ 20 đơn?** → Tách camp Hero → chuyển sang **Mode Target ROI** → vào Phase 2
...
→ Sau đó: chuyển thẳng sang **Mode Target ROI** → vào Phase 2 (không qua GMV check)
...
|---|---|---|
| **1 (cao nhất)** | Video "chết" | → nhảy sang nhánh Critical ngay |
...
**Hành động:** Nhảy sang nhánh Critical → tách các video này ra thành 1 campaign mới, chạy lại từ đầu (Max Delivery phase). Dừng xử lý các kiểm tra còn lại trong ngày.
...
> Mốc 72h có logic riêng, xem [Phase 1](#4-phase-1--72h-learning-max-delivery).
...
Bước 3 — Hero Camp lifecycle:
  - Chờ 24h (không phải 72h như camp gốc)
  - Sau 24h: chuyển sang Mode TARGET ROI
  - Vào vòng lặp 24h bình thường
```
...
FUNCTION run72hLearningPhase(campaign, MDB, ROI_target, budget_initial):
  ...
  IF heroProduct != NULL:
    ...
    campaign.setMode(TARGET_ROI, roi = ROI_target)
...
FUNCTION runOneDayCycle(campaign, ROI_target, MDB):
  ...
  // ================================================================
  // THỨ TỰ ƯU TIÊN:
  //   1. Video "chết"  → nhảy Critical, dừng xử lý các bước còn lại
  ...
  IF deadVideos.isNotEmpty():
    splitDeadVideoCampaign(deadVideos, campaign, MDB)
...
FUNCTION splitHeroCampaign(heroProduct, parentCampaign, ROI_target, MDB):
  ...
  // Hero camp chỉ học 24h (không phải 72h)
  waitUntil(heroCampaign.startTime + 24h)
  heroCampaign.setMode(TARGET_ROI, roi = ROI_target)
...
// ================================================================
FUNCTION splitDeadVideoCampaign(deadVideos, parentCampaign, MDB):
  // ⚠️ CLARIFY: budget_initial dùng công thức MDB 40%/20%?
  //             hay dùng MDB × 50% như hero camp?

  budget = calculateBudgetInitial(MDB)  // TODO: xác nhận công thức
  ...
  // Bắt đầu lại lifecycle từ đầu (72h learning)
  run72hLearningPhase(newCamp, MDB, parentCampaign.roi_target, budget)
...
### ❓ Q2 — Budget của camp "video chết"

Videos từng ≥ 20 đơn nhưng 48h không có đơn mới → tách camp mới.
- Budget dùng công thức `Budget_initial` (40%/20% × MDB)?
- Hay dùng `MDB × 50%` như hero camp?

### ✅ Q3 — Hero camp learning period — ĐÃ XÁC NHẬN

Hero camp chỉ cần **24h** Max Delivery trước khi chuyển sang Target ROI (camp gốc là 72h).
...
1. **Video "chết"** → nhảy sang nhánh Critical ngay
2. **Hero Product xuất hiện** → tách camp Hero
